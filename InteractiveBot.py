...
class InteractiveBot():
    ...
    def handle_suggestion_request(self, setup, story_history):

        if not story_history:
            return self.initial_suggestion(setup)
        else:
            return self.make_suggestion(setup, story_history)

    def make_suggestion(self, setup, story_history):
        ...
        raw_output = self.suggestion_chain.run({
            ...
            "format_instructions": self.format_instructions, "summary": self.memory.load_memory_variables({})["history"]
        })
        parsed_output = self.parse_raw_output(raw_output)

        return parsed_output
    
    def initial_suggestion(self, setup):
